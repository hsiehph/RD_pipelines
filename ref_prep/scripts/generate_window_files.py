...
def get_sliding_regions(mask,wnd_width,slide_by, sunk_based=False):
    ...
    for chr,l in mask.mContigNameLen.items():
...
if __name__=='__main__':
    ...
    for chunk_i in range(len(regions_chrms)):
        print(regions_chrms[chunk_i],regions_wnds[chunk_i].shape[0], file=stderr)
    ...
    for contig, len in n_winds_by_chr.items():
        output_regions_contigs.write("%s\t%d\n"%(contig,len))
    output_regions_contigs.close()

    output_regions=open(o.region_out,"wb")
    pickle.dump([regions_chrms,regions_coords,regions_wnds], output_regions)
